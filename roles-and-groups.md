...
- All Maintainers are required to participate in a nomination, but may effectively abstain from voting by posting a comment that starts with `ABSTAIN` in the PR.
...
- Once the PR has gathered more than half `IN FAVOR` votes out of a total of all `IN FAVOR`, `REJECT`, and missing votes, the PR can be merged and the candidate will be officially part of their new Committer or Maintainer teams.
